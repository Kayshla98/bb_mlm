...
double src_noBB(double Pb, double Ps, TH1D* F){
  ...
  for(i = 1; i <= NBIN; i++){
    ...
    fi = pb * bi + ps * si; //predicted number of events in bin i
    if(di == 0) lnL -= fi;
    else        lnL += di * TMath::Log(fi) - fi;
...
OPTIONS:
  ...
  -v VERBOSITY, --verbosity VERBOSITY
    ...
    if(!strcmp(argv[i], "-t") || !strcmp(argv[i], "--filetype")){
        if(i < argc - 1 && !strcmp(argv[i+1], "vegas")){
            args->filetype = 1;
      }
      if(i < argc - 1 && !strcmp(argv[i+1], "toymc")){
          args->fileytpe = 2;
      }
    }
